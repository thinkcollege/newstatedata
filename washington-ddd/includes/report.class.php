...
UNION SELECT `Value` AS `Option`, `Option1` AS `Value`, `Grouping`  from CombCty ORDER BY `Grouping`, `Option`');
	...
	public function make($displayOnly = false) {
			...
			if (is_array($this->providers)) { 
				if ($this->getCounty() == 'CY91') {
					...
					$geo .= select::fromArray($arr91, 'provider', 'provider');
						...
						else if ($this->getCounty() == 'CY92') {
					$arr91 = array_merge($this->providers['RG01']['CY04'] , $this->providers['RG01']['CY09']);
					$geo .= select::fromArray($arr91, 'provider', 'provider');
					 }
						else if ($this->getCounty() == 'CY93') {
					$arr91 = array_merge($this->providers['RG01']['CY10'] , $this->providers['RG01']['CY33']);
					$geo .= select::fromArray($arr91, 'provider', 'provider');
					 }
					else {
		...
		if ($this->getType() == self::TREND && $this->getStep() == self::STEP_FIRST) {
			...
			$summary->add(new legend('Summary of'), new input(1, input::RADIO, 'summary', 'summary1'), ' ',
				...
				new input(3, input::RADIO, 'summary', 'summary3'), ' ', new label('Average Hours of Support Provided by Activity.', 'summary3'));
	...
	public function exportCSV() {
		...
		$rs = fQuery(str_replace('&dagger;', 'x', $this->generateQuery()));
		$first = true;
		while ($row = $rs->fetchAssoc()) {
			if ($first) {
				$keys = array_keys($row);
	...
	protected function filters() {
		...
		$rs = fQuery('SELECT cl.`code` AS `Value`, cl.`Value` AS `Option` FROM ' . TABLE_CODE_LOOKUP . ' cl
					...
					ORDER BY CASE cl.`Code` WHEN \'DL06\' THEN 0 WHEN \'DL07\' THEN 1 WHEN \'DL02\' THEN 2 ELSE 3 END');
		...
		$rs = fQuery('SELECT cl.`code` AS `Value`, cl.`Value` AS `Option` FROM ' . TABLE_CODE_LOOKUP . ' cl
					   WHERE cl.`grouping` = \'DL\' AND cl.`Code` IN(\'DL06\',\'DL02\',\'DL07\',\'DL09\') AND EXISTS (SELECT 1 FROM ' . TABLE_BILLING . ' WHERE CommunityAccessSupportLevelCode = cl.`Code` LIMIT 0,1)
					ORDER BY CASE cl.`Code` WHEN \'DL06\' THEN 0 WHEN \'DL07\' THEN 1 WHEN \'DL02\' THEN 2 ELSE 3 END');
		$esl = $rs->toSelect('esl', 'All levels');
		$esl->setName('esl');
		$esl->setId('esl');
	...
	protected function generateQuery() {
		...
		if( $this->getCounty() == 'all' ||  $this->getCounty() == '0') {
			$where .= ''; }
			else if ( $this->getCounty() == 'CY91') {
				$where .= ' AND (CountyCode = \'CY03\' OR CountyCode = \'CY11\')'; }
					else if ( $this->getCounty() == 'CY90') {
				$where .= ' AND (CountyCode = \'CY23\' OR CountyCode = \'CY34\')'; }
					else if ( $this->getCounty() == 'CY92') {
	...
	protected function generateColumns($totalrow = 0) {
		...
		// Hack PF --changed #Served query to reflect total ids in system, not hours of support.  Removed this:  COUNT(DISTINCT IF(b.`TotalHoursOfSupport` > 0 OR b.`JobPrepHours` > 0 OR b.`JobDevelopmentHours` > 0 OR b.`JobCoachingHours` > 0 OR b.`RecordKeepingHours` > 0, b.ClientID, NULL)) AS `# Served`,			
			...
			$ret = ($totalrow == 0 ? 'cl.Value AS `Activity`' : '\'Total (unduplicated)\' AS \'Total (unduplicated)\'') . ',
					...
					IF(COUNT(DISTINCT IF(b.`TotalHoursOfSupport` > 0 OR b.`JobPrepHours` > 0 OR b.`JobDevelopmentHours` > 0 OR b.`JobCoachingHours` > 0 OR b.`RecordKeepingHours` > 0, b.ClientID, NULL)) NOT BETWEEN 1 AND ' . MIN_ALLOWED_RECORDS . ', FORMAT(AVG(IF(b.`TotalHoursOfSupport` > 0, TotalHoursOfSupport, NULL)), 2), \'&dagger;\') AS `Avg. Total Hours`,
	...
	protected function generateHeading() {
		...
		if ($this->getStep() != self::STEP_FINAL) {
			...
			switch ($this->summary) {
				...
				case 3:	$heading = 'Average Hours of Support Provided per Activity';	break;
				default: $heading = 'Nothing selected';
				break;
			}
			$heading .= ' for ';
			switch ($tf['u']) {
...
if (!$displayOnly) {
			if ($_REQUEST['report'] == 0) {
				$save = '<form id="save-report-form" action="./?' . $_SERVER['QUERY_STRING'] . '" method="post"><p>Save these report settings for future quick access from the home page:<br />'
					  . '<label for="report-title">Title:</label> <input type="text" id="report-title" value="' . $this->getLegend() . '" size="40">'
					  . ' <input type="button" id="save-report" value="Save" disabled="disabled"></p></form>';
			}
			$url = 'http://' . DOMAIN . DIR . '?' . preg_replace('/&graph=.*$/', '', http_build_query(array_merge_recursive($_GET, count($_GET) > 1 ? array() : $_POST)));
	...
	static public function getSavedReport($id) {
		...
		$row = $rs->fetchAssoc();
		$rpt = has_value($_SESSION, 'baseReport2') && $_SESSION['baseReport2'] instanceof report ? $_SESSION['baseReport2']->copy() : new self($row['type']);
	...
	public function setProvider($provider, $step) {
		if (!empty($provider) && preg_match(REGEX_PROVIDER_NUMBER, $provider, $m)) {
			...
			if ($this->county == 'all') { $countyCode = '1'; }
				...
				else if ( $this->county == 'CY93' ) { $countyCode =  '(`CountyCode` = \'CY10\' OR `CountyCode` = \'CY33\')' ;
				} else {
	...
	public function getProvidersInRegionCounty($r, $c) {
		if (has_value($this->providers, $r) && (has_value($this->providers[$r], $c) || in_array($c, array('CY90','CY91','CY92','CY93')))) {
			if ($c == 'CY91') {
				$provarray91 = array_merge($this->providers['RG02']['CY03'], $this->providers['RG02']['CY11']);
				return $provarray91;
			} else if ($c == 'CY90') {
				$provarray90 = array_merge($this->providers['RG06']['CY23'], $this->providers['RG06']['CY34']);
				return $provarray90;
			} else if ($c == 'CY92') {
				$provarray92 = array_merge($this->providers['RG01']['CY04'], $this->providers['RG01']['CY09']);
				return $provarray92;
			} else if ($c == 'CY93') {

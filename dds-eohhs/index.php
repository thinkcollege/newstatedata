...
$_ENV['PHP_CLASSPATH'] = array(__DIR__ . '/includes/', LIVE ? '/home/statedata/lib-ext/' : '/Volumes/Stuff/umb/lib-ext/');
...
if (LIVE && !$user->check('..' . DIR)) {
	#print "<pre>";
	#print_r($user);
	#print error();
	print "ajax:{$_REQUEST['ajax']}\n";
	#exit;
...
$template->addRegion('title', ' - DDS Individual Job Placement Reporting Tool');
$template->addRegion('top', 'DDS Individual Job Placement Reporting Tool');
//$template->addSideMenuItem('./', 'Project Home');
//$template->addSideMenuItem('./page=update', 'Report Placements');
$template->addSideMenuItem('../users/logout.php', 'Logout');
...
if ($report > 0 || report::validType($report)) {
	...
	if (LIVE && !$user->check('..' . DIR . "ajax/$ajax/")) {
		header('Forbidden', true, 403);
		header('Content-type:text/plain');
		print "ajax:$ajax\n";
		die('"forbidden"');

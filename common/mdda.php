...
class mdda {
    ...
    */
	...
	static function stringEscape($value) {
		$return = '';
	...
	static function getFilterClause($report) {
		...
		if ($f['areaOffice']) { $cleanoffice = self::stringEscape($f['areaOffice']);
...
if ($type == 'numberinactivity2' || $type == 'numberinactivity') $cols .= ', SUM((IFNULL(`HrsIndComp`, 0) = 0) AND (IFNULL(`HrsIndCont`, 0) = 0) AND (IFNULL(`HrsGroupInt`, 0) = 0) AND (IFNULL(`HrsFac`, 0) = 0) AND (IFNULL(`HrsSelfEmp`, 0) = 0) AND (IFNULL(`HrsComNonWork`, 0) = 0) AND (IFNULL(`YN_vol`, 0) = 0) AND (IFNULL(`fac_non_work_partic`, 0) = 0)) AS "No Activity Participation" ';

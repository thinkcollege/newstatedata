...
if ($f['provider']) {
	$provider = dds::getProviderName($f['provider']);
} else {
	$provider = "ALL";
	$providerId = 0;
}
$legend		= dds::getLegendName($variable, $f['region'], $provider, $f['year']);
$colors		= array("9900CC", "FF9900", "993333"); // purple, orange, brown
$data		= dds::getActivityVariableArray($variable);
...
$pcScript	= "graph.setcategories($labels;)\ngraph.setseries((CLR_{$colors[0]})'$legend';$cats;)\n"
			...
			. "<Text>" . dds::getAxisLabel($variable) . "</Text></Textbox>)\n"
...
$html .= "</tbody></table>";
if (!$print) {
	$html .= "<br /><a target='_new' href='charts/activity_2.php?print=1&amp;variable=$variable&amp;" . dds::serializeFilters() . "'>Printer-Friendly Format</a>";
}

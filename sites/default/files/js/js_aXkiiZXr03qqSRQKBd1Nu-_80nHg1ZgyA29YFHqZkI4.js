...
(function($) {
  ...
  Drupal.d3.columnchart = function (select, settings) {
    ...
    var rows = settings.rows,
      ...
      z = d3.scale.ordinal().range(["blue", "red", "orange", "green", "#B0C2A5"]),
...
(function($) {
  ...
  Drupal.d3.statedata = function (select, settings) {
    ...
    var rows = settings.rows,
      ...
      x = d3.scale.linear().domain([0,rows.length]).range([0,chart.w]),
      y = d3.scale.linear().domain([0,max]).range([chart.h, 0]),
      z = d3.scale.ordinal().range(["#219362", "#393295", "#D6B031", "#D66031","#7B9BE4","#EF5189","#5C27F9"]),
      div = (settings.id) ? settings.id : 'visualization';
    ...
    function showToolTip(currentstate,d, i, obj) {
      ...
      bar.attr('stroke', '#ccc')
        .attr('stroke-width', '1')
        .attr('opacity', '0.75');
var w = (currentstate.length + 13) * 6.7;
      var group = d3.select(obj.parentNode);
...
(function($) {
  ...
  Drupal.d3.column = function (select, settings) {
    ...
    x = d3.scale.linear().domain([0,rows.length]).range([0,chart.w]),
    y = d3.scale.linear().domain([0,max]).range([chart.h, 0]),
    z = d3.scale.ordinal().range(settings.series_colours),
    div = (settings.id) ? settings.id : 'visualization';
...
(function($) {
  ...
  Drupal.d3.line = function (select, settings) {

    var labels = [],
    key = settings.legend,
    rows = settings.rows,
    p = [10, 50, 70, 50],
    w = settings.width - p[1] - p[3],
    h = settings.height,
    // Setting for background color.
    bgColor = settings.colour,
    // Setting showing legend.
...
(function($) {
  ...
  Drupal.d3.donut = function (select, settings) {
...
(function($) {
  ...
  Drupal.d3.bubble = function (select, settings) {
